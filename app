...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Share+:root {
--arcade-bg: #0a0a12;
...
h3 { font-family: 'Orbitron', monospace !important; color: var(--arcade-text) !important; font-/* ── Game card ── */
.game-card {
...
font-family:'Share Tech Mono',monospace; font-size:0.6rem; font-weight:700; letter-spacing:.badge-want { background:rgba(245,197,24,0.15); color:var(--arcade-yellow); border:1px solid .badge-played { background:rgba(57,255,20,0.12); color:var(--arcade-green); border:1px solid
.badge-norom { background:rgba(0,229,255,0.10); color:var(--arcade-cyan); border:1px solid .badge-noplay { background:rgba(255,45,120,0.12); color:var(--arcade-magenta);border:1px solid .badge-none { background:rgba(107,107,138,0.15);color:var(--arcade-dim); border:1px solid /* ── Detail panel ── */
.detail-title {
font-family: 'Orbitron', monospace;
font-size: 1.1rem;
font-weight: 900;
...
.current-status-label { color: var(--arcade-dim); text-transform: uppercase; font-size: 0.6rem; /* ── Stats bar ── */
.stats-bar {
display: flex;
gap: 0;
background: var(--arcade-panel);
...
.stat-num { font-family:'Orbitron',monospace; font-size:1rem; font-weight:700; color:var(--arcade-.stat-label { font-family:'Share Tech Mono',monospace; font-size:0.58rem; color:var(--arcade-/* ── Scanline overlay ── */
[data-testid="stMain"]::before {
content:''; position:fixed; top:0;left:0;right:0;bottom:0;
background:repeating-linear-gradient(0deg,rgba(0,0,0,0.025) 0px,rgba(0,0,0,0.025) 1px,transparent pointer-events:none; z-index:9999;
}
/* ── Widget overrides ── */
...
[data-testid="stDataFrame"] { border:1px solid var(--arcade-border) !important; border-radius:</style>
...
# ----------------------------
CABINET_SUMMARY = (
"Your cabinet: 4-way stick + 8-way stick, 6 buttons/player, NO spinner/trackball/lightgun/"horizontal monitor (vertical OK)."
)
BLOCKED_GENRE_EXACT = {
...
VALUES (?, ?, datetime('now'))
ON CONFLICT(history_key) DO UPDATE SET history_md=excluded.history_md, updated_at=datetime('""", (history_key, history_md))
conn.commit()
conn.close()
...
if not api_key:
raise RuntimeError(
"Missing OPENAI_API_KEY. Create .streamlit/secrets.toml or set OPENAI_API_KEY env )
payload = {
...
headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json", "Accept": method="POST",
)
try:
...
q = game_name.replace(" ", "+")
return {
"Gameplay (YouTube)": f"https://www.youtube.com/results?search_query={q}+arcade+"History / Legacy (search)": f"https://www.google.com/search?q={q}+arcade+history+legacy",
...
"Ports / Collections": f"https://www.google.com/search?q={q}+arcade+collection+}
...
want_roms = {rom for rom, status in st.session_state.status_cache.items() if status == STATUS_if not want_roms:
...
return cache[url]
try:
req = Request(url, headers={"User-Agent": "Mozilla/5.0 (ArcadeGamePicker)"}, method="with urlopen(req, timeout=timeout_sec) as resp:
b = resp.read()
cache[url] = b
return b
except Exception:
...
s = x.strip()
if (s.startswith("http://") or s.startswith("https://")) and re.search(r"\.(png|jpg|urls.append(s)
walk(obj)
...
for k in ("title","description","manufacturer","year","genre","players","buttons","controls","if k in data and data[k]:
val = data[k]
...
meta_items = []
if y: meta_items.append(f'<div class="detail-meta-item"><strong>Year</strong><span>{if c: meta_items.append(f'<div class="detail-meta-item"><strong>Company</strong><span>{if genre: meta_items.append(f'<div class="detail-meta-item"><strong>Genre</strong><span>{if platform: meta_items.append(f'<div class="detail-meta-item"><strong>Platform</strong><if rom: meta_items.append(f'<div class="detail-meta-item"><strong>ROM</strong><span st.markdown(
...
with h1:
generate_btn = st.button(" Generate", use_container_width=True, key=f"hist_gen_{with h2:
refresh_btn = st.button(" Regenerate", use_container_width=True, key=f"hist_refresh_{with h3:
clear_btn = st.button(" Clear saved", use_container_width=True, key=f"hist_clear_{if clear_btn:
...
st.markdown(f"""
<div class="stats-bar">
<div class="stat-item"><span class="stat-num">{total_games:,}</span><span class="stat-label"><div class="stat-item"><span class="stat-num">{total_visible:,}</span><span class="stat-label"><div class="stat-item"><span class="stat-num">{played_count}</span><span class="stat-label"><div class="stat-item"><span class="stat-num">{want_count_v}</span><span class="stat-label"></div>
""", unsafe_allow_html=True)
# ----------------------------
# Two-panel layout
# ----------------------------
...
st.session_state.selected_key = game_key(pd.Series(st.session_state.picked_rows[st.rerun()
...
if st.button("▶ Open", key=f"card_open_{rom_val}_{int(row['year'])}", use_container_st.session_state.selected_key = game_key(row)
st.rerun()
if len(hits) > card_limit:
st.caption(f"Showing first {card_limit} of {len(hits):,}. Use search or filters # Selectbox for full list
st.markdown("##### Or select from full list")
view = hits[["rom", "game", "year", "company", "genre", "platform"]].copy()
view["status"] = view["rom"].apply(lambda r: STATUS_LABELS.get(status_for_rom(str(r).if len(view) > 0:
labels = (
...
st.markdown("""
<div style="text-align:center; padding:40px 20px; color:#6b6b8a; font-family:'Share Tech <div style="font-size:2.5rem; margin-bottom:12px;"> </div>
