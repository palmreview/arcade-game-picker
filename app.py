...
Your job is to write a concise but insightful historical profile for a single arcade game.
Focus on:
- cabinet experience
- gameplay
- historical context
...
- If a fact is uncertain or not well documented, say that clearly.
- Do not invent cabinet production numbers, sales figures, or developer stories.
- If the game is obscure, explain that and still provide useful context.
- Write in clean markdown with short sections and bullets where helpful.
"""
...
def generate_history_profile(game_name: str, year: int, company: str, genre: str, platform: str, rom: str) -> str:
    ...
    if not api_key:
        raise RuntimeError(
            "Missing OPENAI_API_KEY. Add it to Streamlit secrets or your environment before using History Mode."
        )
    ...
    try:
        ...
        raise RuntimeError(f"OpenAI API error ({e.code}): {err_text}") from e
...
def show_game_details(row: pd.Series):
    ...
    st.markdown("---")
    st.markdown("### 📚 History Mode")
    st.caption("Generate a short historical profile for this game using OpenAI.")
    ...
    with h1:
        generate_btn = st.button("🧠 Generate History Profile", use_container_width=True, key=f"hist_gen_{history_key}")
    with h2:
        refresh_btn = st.button("♻️ Regenerate", use_container_width=True, key=f"hist_refresh_{history_key}")
    ...
    if generate_btn or refresh_btn:
        with st.spinner("Generating history profile..."):
            try:
                ...
                st.session_state.history_error_cache[history_key] = str(e)
                existing_error = str(e)

    if existing_error:
        st.error(existing_error)
        st.caption("Tip: add OPENAI_API_KEY to Streamlit secrets before using History Mode.")
